...
slides/
├── index.html
├── CONTENT.md           # User's content editing surface (slide content in markdown)
├── package.json
├── vite.config.ts
├── tailwind.config.ts
├── tsconfig.json
...
│   │   ├── SectionDivider.tsx   # Phase/section divider (phase, title, timeline props)
│   │   ├── CodeBlock.tsx        # Styled code display with optional language label
│   │   ├── AnimatedText.tsx     # Reusable text entrance animations
│   │   ├── Chart.tsx            # Recharts wrapper (if needed)
│   │   ├── ImageSlide.tsx       # Image display component (if needed)
│   │   └── ...
│   └── lib/
...
- **Auto-advancing reveal:** Use `useEffect` + `setTimeout` to auto-advance steps (e.g., 1.2–2.5s per step). Great for flow diagrams, threat lists, pipeline steps.

### Advanced Animation Patterns Catalog

Use these patterns to make slides more visual and engaging. Prefer these over static text/tables.

#### CountUp Numbers
Animate statistics counting up from 0. Use `useMotionValue` + `useTransform` + `animate` from framer-motion:
```tsx
...
```
**Use when:** Slide contains impressive numbers (stats, counts, percentages).

#### TypewriterCode
Reveal code character by character with a blinking cursor:
```tsx
function TypewriterCode({ code }: { code: string }) {
  const [len, setLen] = useState(0);
  useEffect(() => {
    if (len < code.length) {
      const t = setTimeout(() => setLen(l => l + 1), 18);
      return () => clearTimeout(t);
    }
  }, [len, code.length]);
  return (
    <div style={{ fontFamily: "'JetBrains Mono', monospace", whiteSpace: 'pre', ... }}>
      {code.slice(0, len)}
...
```
**Use when:** Showing code snippets, CLI commands, config files.

#### SVG Ring Gauge
Animated circular progress ring for percentages:
```tsx
function AnimatedGauge({ percentage, label, color }: { percentage: number; label: string; color: string }) {
  const r = 60, c = 2 * Math.PI * r;
  return (
    <svg width={140} height={140} viewBox="0 0 140 140">
      <circle cx="70" cy="70" r={r} fill="none" stroke="rgba(255,255,255,0.06)" strokeWidth="8" />
      <motion.circle cx="70" cy="70" r={r} fill="none" stroke={color} strokeWidth="8"
        strokeLinecap="round" strokeDasharray={c}
        initial={{ strokeDashoffset: c }} animate={{ strokeDashoffset: c - (percentage / 100) * c }}
        transition={{ duration: 1.5, ease: [0.25, 0.1, 0.25, 1] }}
        transform="rotate(-90 70 70)" />
      <text x="70" y="70" textAnchor="middle" dominantBaseline="central"
        fill="white" fontSize="24" fontWeight="700">{percentage}%</text>
    </svg>
  );
}
```
**Use when:** Comparing percentages, success rates, scores side by side.

#### Animated Before/After Bars
Show comparison with growing horizontal bars:
```tsx
<motion.div
  initial={{ width: 0 }}
  animate={{ width: '35%' }}
  transition={{ duration: 1, delay: 0.5, ease: [0.25, 0.1, 0.25, 1] }}
  style={{ height: 32, background: 'var(--accent)', borderRadius: 4 }}
/>
```
**Use when:** Showing cost savings, context window usage, before/after comparisons.

#### Pulsing Glow Labels
Subtle attention pulse on important labels:
```tsx
<motion.span
  animate={{ boxShadow: ['0 0 0px rgba(67,97,238,0)', '0 0 8px rgba(67,97,238,0.3)', '0 0 0px rgba(67,97,238,0)'] }}
  transition={{ duration: 2, repeat: Infinity, ease: 'easeInOut' }}
/>
```
**Use when:** Highlighting key terms, status labels, type badges.

#### Auto-Flipping Comparison
Toggle between two states (e.g., bad vs good) with AnimatePresence:
```tsx
const [showGood, setShowGood] = useState(false);
useEffect(() => { const t = setTimeout(() => setShowGood(s => !s), 3000); return () => clearTimeout(t); }, [showGood]);
```
**Use when:** Good/bad examples, before/after, comparison slides.
...
3. **Scaffold the app** — set up Vite, Tailwind, Framer Motion, project structure, theme from preset, shared components (including SectionDivider and CodeBlock — these are almost always needed)
4. **Create `CONTENT.md`** — a markdown file in the project root with `## Slide N: Title` placeholders. This is the user's content editing surface. If the user provides content upfront, skip this and build directly.
5. **Start dev server** — run `npx vite --host` in background so the user can preview immediately
6. **Build slides from content** — once content is available:
   - For **10+ slides**: launch **3-4 parallel Agent calls**, each building a batch of 5-8 slides simultaneously. Give each agent full context (imports, CSS variables, component APIs, animation patterns).
   - For **<10 slides**: build sequentially in one pass.
7. **Wire up `slides/index.ts`** — import all slides and verify build (`tsc --noEmit` + `vite build`)
8. **Visual enhancement pass** — review all slides and suggest visualization upgrades:
   - Tables with numbers → Recharts bar/area charts
   - Percentages → SVG ring gauges
   - Comparisons → before/after bars or auto-flipping cards
   - Processes/flows → step-by-step progressive reveal
   - Statistics → CountUp hero numbers
   - Code blocks → TypewriterCode reveal
   - Decision frameworks → visual spectrum diagrams
9. **Iterate** — adjust based on feedback
10. **Images** — when a slide needs an image, tell the user what image is needed and where to place it (`public/images/`). Use a visible placeholder with the filename so they know what's missing.

### Parallel Agent Execution (for 10+ slides)

When launching agents to build slides in parallel, give each agent:
- Full list of CSS variables and their values
- Import patterns (`framer-motion`, not `motion/react`)
- Component APIs (SectionDivider props, CodeBlock props)
- Animation utility imports (staggerContainer, staggerItem, scaleFade, etc.)
- The specific slides to build with full content

Example: for a 25-slide deck, launch 3 agents building slides 7-11, 12-18, and 19-25 simultaneously.

### Visualization-First Principle

**Always prefer visual representation over text.** When reviewing content:
- A chart beats a table
- A gauge beats a number
- A diagram beats a bullet list
- An animated flow beats a static description
- A spectrum beats a decision matrix

If a slide is just bullets + heading, ask: "Can this be a chart, diagram, or visual comparison instead?"

### Critical Reminders

- **Import `framer-motion`**, never `motion/react` (doesn't exist in v12+)
- Always keep the app in a runnable state. Never leave broken imports or missing files.
- Run `npx tsc --noEmit` and `npx vite build` after each batch to catch errors early.
- SectionDivider and CodeBlock components should be scaffolded upfront — they're used in almost every deck.
